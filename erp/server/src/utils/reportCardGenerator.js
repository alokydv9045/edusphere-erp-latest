...
const generateReportCardPDF = async (data) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            // School branding
            const schoolName = schoolConfig?.schoolName || process.env.SCHOOL_NAME || 'EduSphere School';
            ...
            if (showAttendance) {
                ...
                doc.text(`Attendance: 95%`, 350, summaryY + 35); // Static for now
            }

            if (showRemarks) {
                doc.fontSize(10).font('Helvetica-Oblique');
                doc.text(`Class Teacher Remarks: Excellent performance, keep it up!`, 60, summaryY + 95, { width: 480 });
            }
            ...
            doc.text('Generated by EduSphere ERP on ' + new Date().toLocaleDateString(), 50, footerY + 40, { align: 'center', width: 500 });
